...
CONTEXT HANDOFF: Read `memory/handoffs/2026-07-25-emergency-handoff.yaml` for full session state.

Quick summary:
- Working on: OKOA Capital website chat + voice AI-agent technology-stack research; deliverable is an updated TOP-5 stack recommendation (run via /acos-research-riffs; two riff sessions under .acos/riffs/).
- Last action: completed LIVE web-verification of the recommendation's load-bearing claims; rescued the six verify-*.md fact-checks into research/okoa-voice-verify-2026-07-25/.
- Next step: render the boss-facing one-page TOP-5 comparison (pieces, costs, and the two must-test items), and/or write the build-and-benchmark plan for the #1 pick (Cartesia cascade over LiveKit/Pipecat + Telnyx + Cloudflare Durable Objects).
- Blockers: none blocking the writeup. Two must-BENCHMARK unknowns for any option: real live-call latency, and 8kHz telephony-band voice quality (no vendor publishes it).

Corrected constraints (do NOT revert): self-host is OPTIONAL; cost is NOT the priority; OKOA is NOT a regulated lender (no compliance framing); brand-voice cloning desired; accurate number read-back needed; website + phone share ONE consistent voice. Ignore the stale cost-priority cloudseat.json/costdim.json draft flagged in the handoff.

Method going forward: verify-first, assume-nothing, use only stated requirements.
...
- HEAD: `1c127b46a31c`
- uncommitted changes: 151 file(s)
