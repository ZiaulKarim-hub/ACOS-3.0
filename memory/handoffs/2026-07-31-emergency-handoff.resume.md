...
CONTEXT HANDOFF: Read `/Users/zee/Documents/Vibe Coding/ACOS 3.0/memory/handoffs/2026-07-31-emergency-handoff.yaml` for full session state.
It supersedes 2026-07-30-emergency-handoff-2.yaml. Every claim in it was independently verified against disk by the handoff agent.

Quick summary:
- Working on: OKOA LinkedIn poster design in ~/.claude/skills/acos-linkedin-posts/ — plus carried-over Rubin document-production loose ends. Nothing this session touched the ACOS 3.0 repo's own code.
- Last action: Drew four building/construction silhouette SVG textures from scratch (OKOA's brand library has NO building artwork — only topographic maps and US state vectors) and showed Zee all four at /Users/zee/Documents/OKOA/LinkedIn Text Posters/silhouette-options/. Recommended S3 (line art); S2 runner-up.
- Next step: Zee picks S1 / S2 / S3 / S4. Then install the chosen SVG into ~/.claude/skills/acos-linkedin-posts/assets/ and point build-text-poster.ts's TOPO_DEFAULT at it, so every text poster uses it with no --texture flag.
- Blockers (all are decisions only Zee can make):
  1. Silhouette texture pick — LIVE, this is the immediate one.
  2. Anchorage closing poster: white vs dark vs coral-band badge. All three FINAL renders exist at "LinkedIn Closing Posters/2026-07-31 Anchorage/". Archived as li-2026-07-31-01, commit 1364d48.
  3. Open boundary question in memory/feedback_no_method_or_scope_in_outbound_docs.md — does the no-scope rule reach sections describing OKOA's OWN underwriting work, or only the AI's search scope?
  4. Whether to regenerate "25th Ave Credit Memo — Source Verification.pdf" (still documents figures the post-facto audit removed).
  5. Rubin Dropbox README.md H1 still reads "# Fraud Case Additional Documents — Rubin"; folder is "Rubin Lawsuit Additional Documents". Fix before any counsel email.
  6. Origination fee UNRECONCILED: $107,000 (Zee) vs $135,800 (2% per 12/23/2021 term sheet).
  7. CARRIED OVER: Thurston CHANGE NOTE 20260728.md section 2.1 (option 1/2/3 unpicked); Phil Wolfgramm stale-path issue; Casa Grande I release not located.

Standing rules established THIS session — apply them, do not re-derive:
- NO scope statements, provenance, or any account of what the AI searched in ANY document going to Zee's boss. That material goes in a SEPARATE companion doc for Zee only. Relocate it, never delete it. (memory/feedback_no_method_or_scope_in_outbound_docs.md)
- Text posters use "V5 house style": --layout list --badge dark, ONE version. This deliberately OVERRIDES the render-both-badges rule, which still applies to CLOSING posters only.
- A closing poster publishes a BORROWER'S deal → consent gate blocks, deal facts allowed. A text poster publishes OKOA'S OWN commentary → no consent gate, but okoa-positioning.md's blocklist applies IN FULL and is enforced in code. Do not carry habits between them.

Do NOT re-derive facts the handoff already carries with sources. All refusal guards were verified passing (5/5 closing, 8/8 text-poster) — re-run them only if you change those scripts.
...
- HEAD: `551301a42576`
- uncommitted changes: 199 file(s)
...
?? memory/handoffs/2026-07-30-emergency-handoff-2.resume.md
?? memory/handoffs/2026-07-30-emergency-handoff-2.yaml
?? memory/handoffs/2026-07-30-emergency-handoff.yaml
?? memory/handoffs/2026-07-31-emergency-handoff.yaml
