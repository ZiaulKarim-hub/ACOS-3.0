...
### 4.8 `legal-analyst` can never be auto-assigned — **Medium** — ✅ **FIXED 2026-06-04**
...
> **FIX (advisor path):** `assign-reviewers.sh` now also scans advisory rule files (any `*.yaml` that isn't a gating `*-reviewer.yaml` or `global.yaml`), evaluates their triggers with the same `_matches()` logic, and writes matches to `.acos/state/review-advisors/<slice>.json` — **without** adding them to the gating reviewer array (so an advisor never has to emit PASS/REJECT). `acos-execute-slice` Step 10 reads that file and spawns each advisor as **non-gating diligence** (surfaced, never blocking). Verified end-to-end: a slice touching `loan/`, `guaranty/`, `deed` → reviewers `["qa-reviewer"]` + advisors `["legal-analyst"]`; a non-legal slice → advisors `[]`. (Your only remaining part: keep `legal-analyst.yaml`'s triggers current — confirmed already covering the core legal surface.)
...
### 6.3 Agent-performance metrics are permanently blank — **High** — 🟡 **PARTIALLY FIXED 2026-06-04**
...
> **FIX (root cause):** Captured a live SubagentStop payload and found the hooks read a non-existent `agent_name` field — the real fields are **`agent_type`** + **`agent_id`**. Fixed `log-agent-completion.sh` and `log-agent-spawn.sh` to use them. Verified: new entries now read `COMPLETED general-purpose <id>` instead of `unknown`. **Still open:** the ~1,972 historical `unknown` lines are unrecoverable, and nothing yet *computes* effectiveness metrics from the log (the `agent-effectiveness/` dirs + QAP/TER/UAPS formulas remain unbuilt — a downstream analysis step, related to preeng §0.5).
