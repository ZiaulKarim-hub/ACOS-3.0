...
if [ -d "memory" ]; then
    ...
    HANDOFFS=$(find memory/handoffs \( -name "*.yaml" -o -name "*.md" \) -not -path '*/archive/*' 2>/dev/null | wc -l | tr -d ' ')
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    find memory/handoffs \( -name "*.yaml" -o -name "*.md" \) -not -path '*/archive/*' ! -name ".template.yaml" ! -name ".template.md" ! -name '*.resume.md' -delete 2>/dev/null || true
