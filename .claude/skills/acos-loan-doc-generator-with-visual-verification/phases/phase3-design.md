...
**HARD ITERATION CEILING:** Regardless of `config.max_iterations`, if
`current_iteration > 10`, ABORT immediately with an error message:
...
If Phase 4 validation returns FAIL, the Wigum loop restarts Phase 3.
Phase 4 now has THREE gates — any gate failure triggers a new iteration:

1. **Data/code failures** — Read the PPTX validation report at:
...
2. **Layout failures** — Read the layout check report at:
   `{session_dir}/visual-audit/iteration-{N}/layout-check.yaml`
   - Shape boundary violations: fix coordinates directly
   - Shape overlap: separate overlapping elements by adjusting positions
   - Text overflow: increase container height or reduce text content
   - Margin breach: reduce width of offending shapes
   - Cross-slide inconsistency: align elements across all slides

3. **Visual failures** — Read the visual defect report at:
   `{session_dir}/visual-audit/iteration-{N}/visual-defects.yaml`
   AND read the screenshots at:
   `{session_dir}/visual-audit/iteration-{N}/slide_NN.png`
   - Color issues: adjust palette in design spec
   - Spacing issues: adjust positions/gaps
   - Typography issues: fix font assignments
   - Hierarchy issues: adjust element sizing
   - Balance issues: redistribute content across slides

**Build-time guard rails** (apply on every iteration):
Before generating the PPTX, validate each element placement:

a) **Container auto-sizing:** For every text shape, estimate required height:
   ```
   char_width ≈ font_size_emu * 0.6  (conservative estimate)
   chars_per_line = container_width / char_width
   num_lines = ceil(len(text) / chars_per_line)
   required_height = num_lines * (font_size_emu * 1.3) + padding
   if required_height > container_height:
       container_height = required_height  (auto-expand)
   ```

b) **Overlap pre-check:** After placing all elements on a slide, check every
   pair of shapes for bounding-box intersection. If overlap detected, adjust
   the later element's position down by the overlap amount + 50000 EMU gap.

c) **Margin guard:** Before finalizing any shape, verify:
   - left >= 0
   - top >= 0
   - left + width <= slide_width
   - top + height <= slide_height
   If any violation: clamp to bounds and log a warning.

d) **Footer clearance:** No content element's bottom edge should be within
   100000 EMU of the footer divider line. If too close, reduce element height.

After applying guard rails, regenerate by calling data-to-pptx.py with corrected
inputs. Then proceed to Phase 4 for all three validation gates.

Maximum iterations controlled by `config.max_iterations` (default 5 for
visual-verification-enabled pipeline, up from 3).
...
─────────────────────────
The assembled document uses a Carbon 16-column grid. Structure your section HTML as:
<div class="row">
...
For multi-column layouts within your section:
- Two equal columns: <div class="col-8">A</div><div class="col-8">B</div>
- Sidebar + main: <div class="col-5">sidebar</div><div class="col-11">main</div>
- Three cards: <div class="col-5">card</div> repeated 3x

Use .row--condensed (16px gap) for data-dense layouts.
Use .row--narrow (1px gap) for data callout rows.
Wrap narrative paragraphs in: <div class="col-8 col-offset-4"> for optimal reading width.
...
5b. Use <div class="section-label">NN — SECTION NAME</div> for section labels (IBM Plex Mono, uppercase, coral accent).
