...
DEFAULT STATUS VERIFICATION (CRITICAL):
- Do NOT infer default status from the existence of draft default letters, forbearance
  correspondence, or overdue payment notices alone. A loan can have overdue payments
  without being in formal default.
- Default status must come from: (a) the loan servicing system (e.g., HyperCore
  "Mark as Defaulted" status), or (b) explicit user confirmation.
- Add a `loan_status` section to loan-data.yaml with:
    loan_status:
      default_confirmed: false    # true only if servicing system or user confirms
      default_source: "none"      # "servicing_system", "user_confirmed", or "none"
      overdue_payments: true/false # factual — are there late payments?
      default_letters_found: true/false  # factual — were draft default letters in the folder?
      notes: "Overdue payments exist but formal default not recognized by lender."
- If the loan folder contains default-related documents but no servicing confirmation,
  set default_confirmed: false and flag for user verification.
...
MANDATORY VERIFICATION ROW — Default Status:
Always include this entry in the verification table:
  data_point: "Loan Default Status"
  value: "Performing" or "In Default"
  type: "requires_confirmation"    # NOT extracted — must be confirmed
  source_document: "N/A — requires servicing system or user confirmation"
  confidence: 0.0                  # zero until confirmed
  cross_validated: false
  section_used_in: ["Executive Summary", "Risk Factors", "Recovery Analysis"]
  verification_note: |
    Default status cannot be inferred from draft default letters or overdue payment
    notices alone. Please confirm: Is this loan formally in default per the servicing
    system, or is it performing with overdue payments?
...
- type is "requires_confirmation" (mark as "⚠ REQUIRES USER CONFIRMATION")
