# acos-synthesis-protocol

The **execution half** of `acos-genesis-protocol`. The planning engine produced a tree of
...
build-plan.json (leaves first)
   ...
   │            fail ▶ Builder reworks (≤ max_iter)
   │            functional pass ▶ HARDENING GATE (code leaves only): Hardener(s) review → Builder
   │              fixes → re-verify, ↺ until clean ≥ severity-gate (or rounds cap → punch-list)
   │            ▶ status=passed, library refreshed
...
The roles map onto ACOS's adversarial trust model:
**Builder ≈ developer (also the hardening fixer)**, **Verifier ≈ qa-reviewer (zero-trust, fresh
agent)**, **Hardener ≈ qa-reviewer (read-only worktree; scoped `acos-robust-code-review`)**,
**Integrator ≈ architect (composition + repair)**. All spawned as **general-purpose** agents with
embedded prompts (`prompts/{builder,verifier,hardener,integrator}.md`) — no new `.claude/agents/`
files. A code leaf is *mechanically* unable to reach `passed` un-hardened (`set-status.py` exit 4),
so every code component is bulletproof before it is composed upward.

## Invocation

```
/acos-synthesis-protocol <feature-dir>                 # full bottom-up build
/acos-synthesis-protocol <feature-dir> --component <C> # one component (+ its deps)
/acos-synthesis-protocol <feature-dir> --status        # render + print status, stop
/acos-synthesis-protocol <feature-dir> --max-iter <N>  # rework cap (default 5)
```

`<feature-dir>` = `planning/preeng-unix/<feature-id>` (must pass
`acos-genesis-protocol/scripts/verify-artifacts.sh` first).
...
change (the runtime re-runs `acos-genesis-protocol/scripts/render-library.py`). Open the library and use
...
```
.claude/skills/acos-synthesis-protocol/
