---
name: acos-synthesis-protocol
description: Bottom-up execution engine for acos-genesis-protocol component trees. Walks the build plan leaves-first — builds each component's observable output, verifies it (pluggable auto-check + human test) in isolation, registers pass/fail in the browsable Component Library, then composes verified children into their parent (itself a testable component) and verifies the parent. On an integration failure it drills DOWN to the likely-culprit children, upgrades them, and re-climbs (the up→down→up repair loop) until the root Product passes its whole-product verifier against the vision. Spawns Builder / Verifier / Integrator general-purpose agents (no new restricted agent files). Use /acos-synthesis-protocol planning/preeng-unix/<feature-id> after the planning engine's coverage gate passes.
...
This is the **execution half** of `acos-genesis-protocol`. The planning engine produced a tree of
...
build-plan.json (leaves first)
   ...
   │                            └ functional pass ▶
   │                                 HARDENING GATE (code leaves only):
   │                                   Hardener(s) review artifact → Builder fixes findings → re-verify
   │                                   ↺ until zero findings ≥ severity-gate (or rounds cap → punch-list)
   │                                 └ status=passed (clean|punchlist), evidence logged, library refreshed
   │
...
```
/acos-synthesis-protocol <feature-dir>                 # run the full bottom-up build
/acos-synthesis-protocol <feature-dir> --component <C> # build/verify a single component (+ deps)
/acos-synthesis-protocol <feature-dir> --status        # print the tree status, render library, stop
/acos-synthesis-protocol <feature-dir> --max-iter <N>  # override per-component rework cap (default 5)
/acos-synthesis-protocol <feature-dir> --harden-rounds <N>   # hardening-gate review rounds (default: tree's hardening.default_max_rounds, else 2)
/acos-synthesis-protocol <feature-dir> --severity-gate <LVL> # block integration at/above LVL (CRITICAL..TRIVIAL; default MEDIUM)
/acos-synthesis-protocol <feature-dir> --no-harden     # disable the code-hardening gate entirely (functional verify only)
```
...
bash .claude/skills/acos-genesis-protocol/scripts/verify-artifacts.sh "<feature-dir>"
```
...
```bash
BUILDER_MODEL=$(bash .claude/scripts/resolve-agent-model.sh developer)      # Builder  ≈ developer (also the hardening-gate FIXER)
...
HARDENER_MODEL=$(bash .claude/scripts/resolve-agent-model.sh qa-reviewer)   # Hardener ≈ qa-reviewer (read-only reviewer)
```

**Hardening config (B5).** Resolve the gate's parameters once, here, with this precedence:
CLI flag → tree `hardening.*` → built-in default. `--no-harden` forces the gate off for the
whole run (functional verify only). Otherwise: `HARDEN_ROUNDS` = `--harden-rounds` else
`hardening.default_max_rounds` else `2`; `SEVERITY_GATE` = `--severity-gate` else
`hardening.default_severity_gate` else `MEDIUM`; code artifact types = `hardening.code_verifier_types`
else `software-test,data-schema`. `next-ready.py` already computes per-component `hardening_eligible`
from these — trust its flag rather than re-deriving.
...
```bash
python3 .claude/skills/acos-genesis-protocol/scripts/render-library.py "<feature-dir>"
```
...
```bash
python3 .claude/skills/acos-synthesis-protocol/scripts/next-ready.py "<feature-dir>"
```
...
0. **Reuse check first (don't rebuild what already works).** If the leaf is `reuse.reusable`, query
   ...
   ```bash
   python3 .claude/skills/acos-genesis-protocol/scripts/registry.py search --tags "<this leaf's reuse.tags>" --type "<verifier.type>"
   ```
   If a candidate's capability tags + verifier type + contract match this leaf's need, **reuse it
   instead of building**:
   ```bash
   python3 .claude/skills/acos-genesis-protocol/scripts/registry.py link <feature-dir> <id> <registry-id>
   ```
...
4. **On functional FAIL:** re-spawn the Builder with the FAIL reasons (≤
   `repair_protocol.max_iterations_per_component`). Still failing at the cap →
   `set-status.py <dir> <id> failed --note "<blocker>"`, then **continue** to other independent
   `ready` leaves (one stuck component must not stall the rest); surface it in the final report.
5. **On functional PASS — branch on `hardening_eligible` (from `next-ready.py`):**
   - **`hardening_eligible: false`** (non-code artifact, a parent, human verdict, or `--no-harden`):
     record `passed` directly, exactly as before —
     - agent PASS: `set-status.py <dir> <id> passed --source agent --note "<verifier summary>"`
     - human PASS: `set-status.py <dir> <id> passed --source human --observed "<observed>" --note "<criteria met>"`
   - **`hardening_eligible: true`** (a code leaf): **DO NOT mark it `passed` yet** — run the
     **hardening gate (Step 2a.5)**. Only its outcome marks the component `passed` (with the
     `--hardening` flag). `set-status.py` will mechanically refuse a bare `passed` on an eligible
     code leaf (exit 4), so the gate cannot be skipped by accident.
6. After recording, re-run `next-ready.py` for the new frontier.

#### 2a.5 The hardening gate (code leaves only — scoped robust-code-review)
Runs **after functional PASS, before `passed`**, on a component whose `hardening_eligible` is true.
It is a tightly-scoped slice of `acos-robust-code-review`: review the *single* component's built
artifact, fix findings, re-review, until the artifact is clean at/above the severity gate. Because
the target is one small artifact, it converges in 1–2 rounds — nothing like reviewing a whole
codebase.

```
round = 1
while round <= HARDEN_ROUNDS:
  • Spawn 1–3 Hardeners (general-purpose, $HARDENER_MODEL, isolation: worktree, run_in_background)
    with prompts/hardener.md + the component spec + its contract + acceptance_criteria + the built
    artifact + injected .acos/config/known-design-choices.md. Worktree isolation = reviewers
    mechanically cannot edit. Scale by artifact size (1 for a small file; up to 3 for a multi-file
    leaf — practical parallelism ceiling).
  • Collect YAML findings, DEDUPLICATE (same file + overlapping lines + same category). When in
    doubt keep both.
  • Partition by severity gate:
      blocking  = findings at/above SEVERITY_GATE
      punchlist = findings strictly below SEVERITY_GATE
  • if blocking == 0:  break            # gate satisfied
  • if round == HARDEN_ROUNDS:          # cap hit with blocking findings still open
      break                              # escalate — see below
  • Spawn the Builder ($BUILDER_MODEL) as FIXER with the blocking (and, cheaply, punchlist)
    findings as its rework reasons. It edits ONLY this component's artifact.
  • Re-run the component's auto_check (functional regression guard — a fix must not break the
    acceptance test). If it regressed, treat as a functional FAIL and Builder reworks.
  • round += 1
```

Record the result on the `passed` write (the verdict is still gated on the **functional** PASS;
hardening adds the `--hardening` state):
- **Clean** (zero blocking, zero punchlist): `set-status.py <dir> <id> passed --source agent --hardening clean --note "verify PASS; hardened clean in <r> round(s)"`
- **Punchlist** (zero blocking, some sub-gate findings deferred): write them to
  `<dir>/evidence/<id>-punchlist.md`, then `set-status.py <dir> <id> passed --source agent --hardening punchlist --punchlist evidence/<id>-punchlist.md --note "verify PASS; hardened to ≥<gate>; <k> low/trivial deferred"`. Surface the punch-list in the final report for user approval (do NOT auto-fix design/efficiency items — same split as the framework's autopilot).
- **Cap hit with blocking findings still open**: do **not** force a pass. `set-status.py <dir> <id> failed --note "hardening unconverged after <HARDEN_ROUNDS> rounds: <summary>"` and surface it (this is a genuine quality stop, like a functional FAIL at the iteration cap). The leaf's parent stays `blocked`.

> **Oscillation guard.** If a finding fixed in round N is re-reported in round N+1 (the fix itself is
> flagged), escalate to the orchestrator and record the decision in
> `.acos/config/known-design-choices.md` so it is injected into — and silenced for — every later
> Hardener. Never let a leaf oscillate forever; that is what `HARDEN_ROUNDS` bounds.
...
4. On `PASS`: `set-status.py <dir> <id> passed --source <agent|human> --note "…"` and climb (re-run
   `next-ready.py`). **Parents are NOT code-hardened** (`hardening_eligible` is always false for a
   node with children): a parent has no code of its own — it is pure composition, and its children
   were each hardened before they could reach `passed`. Cross-component issues are caught by this
   composed Verifier (and, if configured, the integration reviewer), not by re-reviewing the parent
   as if it were a code artifact.
...
Hardening: <hardened> code leaves gated  (<clean> clean, <punchlist> with deferred items)
Library: <feature-dir>/library.html

Hardening punch-list (deferred sub-gate findings — your call):
  - <C-id>: <k> LOW/TRIVIAL — see <feature-dir>/evidence/<id>-punchlist.md
  - ...
[If any leaf failed the hardening gate (blocking findings unconverged at the rounds cap), list it
 with its blocker and evidence ref — same prominence as a functional FAIL.]
...
Next:
  ...
  - For a failed component: /acos-synthesis-protocol <feature-dir> --component <C>
```
...
- **Independence of verification:** the Verifier is always a fresh agent that does not see the
  Builder's self-assessment — zero-trust, mirroring ACOS reviewers. The Hardener is likewise fresh
  and runs in a read-only worktree (mechanically cannot edit).
- **Evidence-backed:** every build/verify/harden writes an evidence note; status in the library is
  never set to `passed` without a recorded Verifier `PASS`.
- **No silent assembly:** a parent can never be `passed` while any child is not `passed`.
- **Hardened-before-integration:** a code leaf (leaf whose `verifier.type` ∈
  `hardening.code_verifier_types`) can never reach `passed` until its hardening gate is satisfied
  (`set-status.py` exit 4 otherwise). Because a parent gates on children being `passed`, every code
  component is provably hardened before it is composed upward — no extra parent-side check needed.
  The gate is type-conditional: non-code artifacts (documents, blueprints, measurements) are never
  code-reviewed, so the domain-agnostic claim holds.
- **Bounded repair:** `max_iterations_per_component` (default 5) caps each functional rework loop;
  `HARDEN_ROUNDS` (default 2) caps each hardening loop. Exhaustion escalates to the user (a `failed`
  leaf) rather than forcing a pass or looping forever.
