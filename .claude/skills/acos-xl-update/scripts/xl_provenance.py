...
def _meeting_when(t: dict) -> str:
    ...
    if isinstance(raw, (int, float)):
        dt = datetime.datetime.fromtimestamp(raw / 1000, tz=datetime.timezone.utc).astimezone(MT)
...
def _render_source(src: dict, cache_ok: bool) -> list:
    """Render one meeting source as Markdown lines. Verifies + timestamps the quote."""
    ...
    lines.append(f"    - **{title}** — {when}  ·  {stamp}  ·  _{verify}_")
    lines.append(f"      > “{quote}”")
    return lines


def _render_bullet(b: dict, prior_date: str) -> list:
    text = (b.get("text") or "").strip()
    kind = (b.get("kind") or "new").lower()
    basis = (b.get("prior_week_basis") or "").strip()
    sources = b.get("sources") or []
    tag = {"new": "NEW", "refreshed": "REFRESHED", "carried": "CARRIED"}.get(kind, kind.upper())
    lines = [f"- **[{tag}]** {text}"]
    if kind in ("carried", "refreshed") and basis:
        lines.append(f"    - _Carried from prior week ({prior_date})_: “{basis}”")
    for src in sources:
        lines.extend(_render_source(src, True))
...
def render(spec: dict) -> str:
    ...
    out.append("")
    out.append(f"**Report date:** {report_date}  ·  **Prior week:** {prior_date}")
    out.append("")
    ...
    for sh in spec.get("sheets") or []:
        ...
        for section in ("progress", "blockers"):
            ...
            out.append("")
            for b in bs:
                out.extend(_render_bullet(b, prior_date))
            out.append("")
    return "\n".join(out).rstrip() + "\n"
...
def main(argv=None) -> int:
    ...
    with open(a.out, "w") as fh:
        fh.write(md)
    # emit a tiny audit summary to stdout (counts + any unverified quotes)
    unverified = md.count("NOT LOCATED VERBATIM")
    missing = md.count("NO source")
    print(json.dumps({"out": a.out, "bytes": len(md),
                      "unverified_quotes": unverified, "sourceless_new_bullets": missing}, indent=2))
    return 0
