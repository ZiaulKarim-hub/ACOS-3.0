...
| **Investor / funding figures** | `.claude/scripts/hca-funding.py` | **BUILT** — PER-LOAN: `funding_outstanding` (an investor's outstanding on a loan via `loanFunding.repaymentSchedule.summary.totalOutstanding`, reconciled to $0.01 + provenance-bound) + `funding_commitment` / `funding_participation` / `funding_receivable` (reliable 2-step assetId→loanFundingId). PORTFOLIO (`FundingPortfolioFigure`, across ALL an investor's loans): `portfolio_receivable` (reconciled, full InstallmentComponents identity; verified all 62 funding entities reconcile) + `portfolio_outstanding` (RECONCILED AGGREGATE — pages `loanFundings(fundingEntityId)` to completion and SUMS each position's `totalOutstanding`; every contributor reconciles its 5-component identity AND is provenance-bound to its own Tier-1 record, the page set is completeness-checked, the sum is `bind_aggregate`-verified; REFUSES on any unreconciled contributor or incomplete fetch — never a partial sum) + `portfolio_commitment` / `portfolio_disbursement` / `portfolio_contributed` / `portfolio_active_loans` (single-source). NATIVE-SOURCED: `per_diem_interest` (an investor's daily interest, delivered DIRECTLY FROM HYPERCORE — the repayment schedule's "Int. Daily Accrual" column = `repaymentSchedule.scheduleTable[row].due.interest` on the current interest-accrual row, provenance-bound to its exact array path `$.…scheduleTable[K].due.interest`, re-resolvable; the value is Hypercore's own, never computed when native is available). Selects the latest `isInterestAccrualDate` row at/before `as_of` (default today). CROSS-CHECK (double-check only, never the delivered value): DERIVES the day-count convention from Hypercore's own number (implied = principal × rate/100 ÷ native) — a clean 360/365 reports the convention as CONFIRMED (not assumed); a non-standard value is flagged but the native value is still delivered. FALLBACK (only when no native accrual row): compute `principal × (currentInterestRate/100) ÷ 360`, clearly labelled `computed_fallback` + Actual/360 ASSUMED (`currentInterestRate` is a PERCENT — returns `14` = 14%, so `/100`; basis is outstanding PRINCIPAL not net `total`). Routed by "per diem" / "perdiem" / "daily interest" keywords (stripped from entity resolution); REFUSES on not-funding / bad as_of / (fallback) absent-or-non-positive inputs. Live-verified XL (fundingEntity 3): on Beehive 134 → 6,922,294.60; portfolio receivable → 27,040,395.55; per-diem on Lux II 171 (loanFunding 338, schedule 214570) → native `due.interest` 1,029.236923/day, day-count Actual/360 CONFIRMED by Hypercore (matches the UI's $1,029.24). |
...
| **Data Catalog** | `.claude/skills/acos-hypercore-ask/catalog/` | **BUILT** — a complete map of every value-bearing Hypercore field (numbers + facts) for loans / investor positions / funding entities / borrowers / equities, each with its exact path, reliable working query, value kind, a real probed example, and (where annotated) a human name + synonyms + gotchas + the figure that delivers it. Pipeline: `hca-catalog-harvest.py` (schema → tiered leaf paths, root-boundary model), `hca-catalog-probe.py` (paths → live GraphQL → real example values), a multi-agent annotation+adversarial-verify workflow, `hca-catalog-build.py` (→ `catalog-index.json` machine + `HYPERCORE-FIELD-MAP.md` human), `hca-catalog-lookup.py` (phrase → path/working-query — `--query "per diem"`), `hca-catalog-refresh.py` (re-introspect + diff vs `catalog-paths.json` → flag schema drift). Access reality recorded: loans/loanFundings/fundingEntities reliable; **equities = HTTP 403 forbidden**, **clients = HTTP 500** (schema-mapped, probe deferred); see `catalog/README.md`. Use the lookup to fetch ANY cataloged field, not just the built figures. |
