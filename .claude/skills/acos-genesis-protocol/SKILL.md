...
`acos-genesis-protocol` is a **tertiary planning system** for ACOS. It sits beside `acos-plan`
...
vision ──▶ [RUNNER]──▶ deterministic_prompt
            ...
            [WORKER]  envision → decompose → contract → verify-spec → library → buildplan → coverage
                          │
                          ▼
   planning/preeng-unix/<id>/  (component-tree.json · library.html · plan.html · build-plan.json · …)
                          │  verify-artifacts.sh  (completeness + coverage gate)
                          ▼
            /acos-synthesis-protocol   (bottom-up build → verify → compose → repair)
```
...
realized by its own execution engine (`/acos-synthesis-protocol`).

## Usage

```
/acos-genesis-protocol [product/vision description]   # full pipeline from a description
/acos-genesis-protocol --from-file <path>             # use an existing vision/brief as context
/acos-genesis-protocol --resume <feature-id>          # re-run / update an existing component tree
```
...
```bash
python3 .claude/skills/acos-genesis-protocol/scripts/registry.py search --json   # or --tags "<domain capabilities>"
```
...
```bash
cat .claude/skills/acos-genesis-protocol/prompts/runner.md
```
...
```bash
python3 .claude/skills/acos-genesis-protocol/scripts/render-library.py "planning/preeng-unix/$FEATURE_ID"
```
...
### Step 3.6: Render the narrative Plan (HTML)

Deterministically generate `plan.html` — the human-readable, read-top-to-bottom plan
(vision · success-criteria map · **two complementary diagrams** · build phases · full wiring
table · per-component reference):

```bash
python3 .claude/skills/acos-genesis-protocol/scripts/render-plan.py "planning/preeng-unix/$FEATURE_ID"
```

The two diagrams serve different purposes:
- **3.1 Pipeline data-flow** — a pure-Python inline SVG; modules stacked in pipeline order
  (topologically sorted), showing stage-to-stage data movement. **Fully offline.**
- **3.2 Component connection diagram** — a Mermaid flowchart with components grouped *inside*
  their subsystems, the hard-gate choke point (thick arrows), and external inputs. The renderer
  **prefers a curated, hand-tuned override** at `planning/preeng-unix/<id>/connections.mmd` (used
  **verbatim** — this is the crisp, human-authored diagram); if absent it auto-generates a clean
  data-flow spine from `integration-map.json`. Rendered with **Mermaid v11 from CDN**, so this one
  diagram **needs network** to draw (a deliberate trade for fidelity — the original v11 layout is
  noticeably crisper than the vendored v10 build).

> **Curating the connection diagram.** To pin a beautiful diagram, author/edit
> `planning/preeng-unix/<id>/connections.mmd` (plain Mermaid `flowchart` source). The renderer
> drops it into `plan.html` verbatim. Delete the file to fall back to the auto-generated spine.

`plan.html` is the **narrative companion** to `library.html` (the browsable catalog). Where the
library answers *"show me each component,"* the plan answers *"how do these connect and in what
order do we build them."* Generation is pure Python 3 stdlib with no network/API calls; §3.1 and
all text render offline, while §3.2 fetches Mermaid v11 at view time. Re-runnable any time the
tree/integration map (or `connections.mmd`) changes.

### Step 4: Verify artifact completeness + coverage

```bash
bash .claude/skills/acos-genesis-protocol/scripts/verify-artifacts.sh "planning/preeng-unix/$FEATURE_ID"
```
...
```
/acos-synthesis-protocol planning/preeng-unix/<FEATURE_ID>
```

which walks the build plan bottom-up: build each leaf → auto + human verify → **harden code leaves
(scoped review-fix loop) before they pass** → register status in the Component Library → compose
verified children into their parent → verify the parent → on failure drill down to the
likely-culprit children, upgrade, and re-climb — until the root Product passes its whole-product
verifier against the vision.

> **Code-hardening gate (built in).** Every code leaf (`verifier.type` ∈ `software-test`/
> `data-schema`) passes through a scoped slice of `/acos-robust-code-review` — fresh reviewers in a
> read-only worktree find internal defects (silent failures, edge cases, security) the acceptance
> test misses; the Builder fixes them; re-review until clean at/above the severity gate. A code leaf
> is *mechanically* unable to reach `passed` un-hardened (`set-status.py` exit 4), so every component
> is bulletproof **before** it is integrated. Non-code artifacts (documents, blueprints,
> measurements) are never code-reviewed — the gate is type-conditional, preserving domain-agnosticism.
> Tune via the optional top-level `hardening` config or the `/acos-synthesis-protocol --harden-rounds /
> --severity-gate / --no-harden` flags. This replaces the old `/acos-execute-slice` interop hack.
...
Next steps:
  1. Read the Plan:               planning/preeng-unix/<FEATURE_ID>/plan.html
  2. Open the Component Library:  planning/preeng-unix/<FEATURE_ID>/library.html
  3. Skim component-tree.json + build-plan.json
  4. Run /acos-synthesis-protocol planning/preeng-unix/<FEATURE_ID> to build bottom-up
```

Provide clickable file links to `plan.html`, `library.html`, `component-tree.json`,
`build-plan.json`, and the feature dir.
