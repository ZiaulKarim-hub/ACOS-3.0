...
running inside ACOS as the engine of `acos-genesis-protocol`. (Sibling of the classic preeng
...
the library, the build plan, and the coverage gate. `/acos-synthesis-protocol` does the
building.
...
the `/acos-synthesis-protocol` runtime:
...
(plus the optional `hardening` config — see below).
...
**Optional `hardening` config (top-level).** `/acos-synthesis-protocol` runs a per-component
code-hardening gate — a scoped `acos-robust-code-review` loop on each code leaf before it is
marked `passed`. You normally do **not** need to emit anything: the runtime derives eligibility
(leaf AND `verifier.type` ∈ `software-test`/`data-schema`) and uses default rounds (2) and severity
gate (`MEDIUM`). Emit a top-level `hardening` object only to tune this product —
`{code_verifier_types[], default_max_rounds, default_severity_gate}` — e.g. add a domain-specific
code verifier type to `code_verifier_types`, or raise `default_severity_gate` to `LOW` for
safety-critical software. To force the gate on/off for a *specific* node, set that node's
`hardening.enabled` (true/false); leave it absent to let the runtime decide. Never code-review a
non-code artifact (documents, blueprints, measurements) — that is what the type gate prevents.
