...
interfaces"). It is the planning half; `acos-synthesis-protocol` is the execution half.
...
> 3. `/acos-genesis-protocol` — tertiary, component tree → own execution engine. **(this)**
...
```
/acos-genesis-protocol [vision description]    # full pipeline from a description
/acos-genesis-protocol --from-file <path>      # use an existing vision/brief
/acos-genesis-protocol --resume <feature-id>   # re-run / update an existing tree
```
...
| | `/acos-plan` | `/acos-preeng-classic` | `/acos-genesis-protocol` |
|---|---|---|---|
...
| Execution | `/acos-execute-slice` | `/acos-execute-slice` | **`/acos-synthesis-protocol`** |

## Files

```
.claude/skills/acos-genesis-protocol/
...
Execution half lives in `.claude/skills/acos-synthesis-protocol/`.
...
- **link** — at execution time, when a reusable leaf matches a registry entry, `/acos-synthesis-protocol`
